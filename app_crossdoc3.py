    ...
    *,
    ...
    try:
        ...
        G, meta = build_graph_from_selection(
            ...
            selected_terms,
            scico_cfg=scico_cfg,
            embedding_fn=embedding_provider,        # rename kwarg
...
with st.sidebar:
    ...
    # Global coref & local filters
    st.session_state["auto_run_global_coref"] = st.checkbox(
        "Auto-run Global Coref (Trie) on commit",
        value=st.session_state["auto_run_global_coref"]
    )
    st.session_state["filter_local_coref"] = st.checkbox(
        "Filter local coref by committed phrases",
        value=st.session_state["filter_local_coref"]
    )
    ...
    if doc_mode == "document":
        ...
        if up:
            ...
            if st.button("Run PDF analysis"):
                ...
                cooc_hf = cfg_coref.get("cooc_hf_tokenizer") or "sadickam/sdgBERT"
    ...
    # =========================
        ...
        ss = st.session_state
        cfg_corpus = ss["config_corpus"]  # ensure available for handler below
        ...
        with st.form("flexiconc_ingest_sidebar", clear_on_submit=False):
            st.markdown("### 📂 Corpus ingestion")

            ss["sidebar_pdf_dir"] = st.text_input(
                "PDF folder to ingest",
                value=ss.get("sidebar_pdf_dir", ""),
                help="Path to folder containing PDFs to ingest into FlexiConc."
            )

            ss["sidebar_new_db_path"] = st.text_input(
                "New FlexiConc destination/name (optional)",
                value=ss.get("sidebar_new_db_path", ""),
                help="Optional path or name for new SQLite database. If blank, append to current DB."
            )
            ...
            with c_ingest_b:
                # Allow trie/cooc/both; "off" is handled via thresholds elsewhere if needed
                current_mode = ss.get("sidebar_coref_mode", "trie")
                if current_mode not in ("trie", "cooc", "both"):
                    current_mode = "trie"
                ss["sidebar_coref_mode"] = st.selectbox(
                    "Coref shortlist mode",
                    options=["trie", "cooc", "both"],
                    index=["trie", "cooc", "both"].index(current_mode),
                    help="Select how shortlist candidates are generated during cross-document coreference."
                )

            ingest_submit = st.form_submit_button("Ingest folder into FlexiConc")

        # ---- legacy/relative path support (keep) ----
        ss["source_root"] = st.text_input(
            "Source folder (optional, e.g. /content/en)",
            value=ss.get("source_root", "")
        )

        # -------- Ingestion handler (single, canonical) --------
        if ingest_submit:
            ...
            if not pdf_dir_val:
                ...
                if not pdf_dir_path.exists() or not pdf_dir_path.is_dir():
                    ...
                    current_db_path = Path(ss.get("db_path") or cfg_corpus.get("sqlite_path", "")).expanduser()
                    ...
                    # Ensure parent exists and DB is openable
                    ...
                    if target_db_path is not None:
                        ...
                        # If new DB chosen, update state and config
                        if new_db_val:
                            ss["db_path"] = target_db_path_str
                            cfg_corpus["sqlite_path"] = target_db_path_str
                            ss["sidebar_new_db_path"] = target_db_path_str

                        pdfs = sorted(pdf_dir_path.rglob("*.pdf"))
                        if not pdfs:
                            st.warning("No PDFs found in that folder.")
                        else:
                            cfg_ing   = ss["config_ingest"]
                            cfg_ui    = ss["config_ui"]
                            cfg_coref = ss["config_coref"]
                            cooc_mode_val,  cooc_tok, cooc_warn = resolve_cooc_backend(cfg_coref)
                                            ...
                                            if enable_cooc_sidebar and _HAVE_COOCC_LOCAL:
                                                ...
                                                if full_text:
                                                    vocab, rows_c, norms = build_cooc_graph(
                                                        ...
                                                        topk_neighbors=10,
                                                        mode=cooc_mode_val,
                                                        hf_tokenizer=cooc_hf,
                                                        return_csr=True,  # optional: ensures CSR for upsert_doc_cooc
                                                    )
    ...
    if not hits:
        ...
        for i, h in enumerate(hits, start=1):
            ...
            with st.expander(f"{i}. {doc_id} • chain {chain_id} • score={score:.3f}"):
                ...
                with cB:
                    if st.button(f"Open doc analysis: {doc_id}", key=f"gc_open_{doc_id}"):
                        try:
                            from flexiconc_adapter import load_production_from_flexiconc
                            prod_loaded = load_production_from_flexiconc(db_path, doc_id)
