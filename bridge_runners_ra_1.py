...
def run_ingestion_quick(
    ...
    **extra,
    ...
    # Attach a lightweight coref n-gram index for later use (non-fatal on failure)
    try:
    ...
    if db_path:
        ...
        if callable(build_ngram_trie) and callable(build_cooc_graph):
            ...
            try:
                ...
                # Try common keys used in your pipelines; fall back to empty list
                sentences_for_cooc = (
                ...
                if effective_mode == "none":
                    ...
                    if rows is not None:
                        # Coerce to CSR if SciPy is available; else skip
                        try:
                            from scipy.sparse import csr_matrix, issparse
                            if (not issparse(rows)) or (rows.getformat() != "csr"):
...
*** a/bridge_runners_ra.py
--- b/bridge_runners_ra.py
@@
-# (old imports remain)
+from pathlib import Path
+from typing import List
+
+from make_flexiconc_db import write_text_from_pdf  # reuse original TXT writer
+from flexiconc_adapter import open_db, build_faiss_indices
+
+
    ...
    if args.faiss:
        try:
            ...
            if not models:
                ...
                # FIX: correct kw name is 'models', not 'model_names'
                build_faiss_indices(cx, models=models)
